using Localization.Json.Internal;
...
namespace Localization.Json
{
    public class JsonStringLocalizerFactory : IStringLocalizerFactory
    {
        ...
        private IStringLocalizer CreateJsonStringLocalizer(string resourcesPath, string resourceName)
        {
            var logger = _loggerFactory.CreateLogger<JsonStringLocalizer>();
            return _resourcesType == ResourcesType.TypeBased ? new JsonStringLocalizer(resourcesPath, resourceName, logger) : new JsonStringLocalizer(resourcesPath, logger);
        }

        private string GetResourcePath(Assembly assembly)
        {
            var resourceLocationAttribute = assembly.GetCustomAttribute<ResourceLocationAttribute>();
            return resourceLocationAttribute == null ? _resourcesRelativePath : resourceLocationAttribute.ResourceLocation;
        }
